...
export default function SignupPage() {
  ...
  const createIamUser = !redirectUrl;
  ...
  async function onSubmit(payload: z.infer<typeof FormSchema>) {
    try {
      ...
      const res = await fetch("/api/signup", {
        ...
        body: JSON.stringify({
          ...payload,
          label: createIamUser ? 'iam' : undefined,
          domain: redirectUrl?.startsWith('http') ? new URL(redirectUrl).hostname : undefined,
        }),
      });
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center gap-2 text-center">
        ...
        <p>We sent an email to your inbox for verification.</p>
        <IconCheck className="text-green-600" />
        <Button variant="link" className="w-full" onClick={() => redirect(redirectUrl ? `/auth/signin?redirectUrl=${redirectUrl}` : '/auth/signin')}>
          Sign in
        </Button>
      </div>
    );
